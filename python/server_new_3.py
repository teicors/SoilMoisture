    ...
    try:
        ...
        while True:
            ...
            if tipo == 9999:
                ...
                with con:
#                    cur = con.cursor(mdb.cursors.DictCursor)
                        sql="select elemento from illuminazione.elemento where time(now()) between ora_inizio \
                        and ora_fine and elemento in (select elemento from illuminazione.elemento \
                        where matricola =%s and pulsante=%s);" % (ip, evento)
                        cur.execute(sql)
                        if cur.rowcount>0:
                            rows = cur.fetchall()
                            for row in rows:
