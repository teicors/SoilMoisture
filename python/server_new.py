...
#
#   Dati in arrivo dalla periferia:elemento
...
#            connection.close()
            ...
            connection.send(b'%s %s %s' % (response_proto, response_status,
            ...
            logger.debug("Received data %r", data)
#            tipo=9999
            evento=json_value['evento']
            valore=json_value['valore']
            ip=json_value['ip']
            logger.debug("data received %s %s" % (evento, valore))
            ...
            if evento == 9999:
                ...
                cur.execute(sql)
                if cur.rowcount==0:
                    sql="insert into illuminazione.matricole (ip, tipologia) VALUES ('%s', %s)" % (ip,valore)
                    cur.execute(sql)
                    con.commit()
                ...
                sql="select tipologia from illuminazione.matricole where ip='%s';" % (ip)
                cur.execute(sql)
                if cur.rowcount>0:
                    rows = cur.fetchall()
                    for row in rows:
                ...
                if tipologia == 13:
                    logger.debug("Received UP")
                    sql="insert into illuminazione.evento (elemento, time) \
                        values \
                        ((select elemento from illuminazione.impianto where matricola = \
                        (select matricola from illuminazione.matricole where ip = '%s') \
                        and pulsante='%s'),'%s');" % (ip,evento,valore)
                    cur.execute(sql)  
                    con.commit()                  
                elif tipologia == 12:
                    logger.debug("Received UP or DOWN")
                elif tipologia == 11:
                    logger.debug("Received DOWN")
                elif tipologia == 10:
                    logger.debug("Received DOWN or UP")
                else:
                    logger.debug("Light management")
                    with con:
                            cur = con.cursor(mdb.cursors.DictCursor)
                            sql="select elemento from illuminazione.elemento where \
                            elemento in (select elemento from illuminazione.impianto a, illuminazione.matricole b \
                            where b.ip='%s' and a.pulsante=%s and a.matricola=b.matricola);" % (ip, evento)
                            logger.debug(sql)
                            cur.execute(sql)
                            if cur.rowcount>0:
                                rows = cur.fetchall()
                                for row in rows:
                                    ...
                                    cur.execute(sql)
                                    con.commit()
                                    sql="select mat.ip, im.pulsante, im.elemento, pl.stato from illuminazione.impianto im, \
                                    illuminazione.matricole mat, illuminazione.elemento pl \
                                    where im.elemento=%s and mat.matricola=im.matricola and mat.attiva=1 \
                                    and pl.elemento=im.elemento;" % (elemento)
                                    cur.execute(sql)
                                    rows2 = cur.fetchall()
                                    for row2 in rows2:                   
                                        HOST = '%s' % row2["ip"]
        ...
        for row in rows:
            ...
            try:
                if os.system("ping -c 1 %s" % ip ) == 0:
                    logger.debug ("host %s appears to be up" % ip)
                    sql="update illuminazione.matricole set attiva=1 where ip='%s';" % (ip)
                    cur.execute(sql)
                    con.commit()
                    logger.debug("set attiva=1")
            except:
                print ("Ping Error:", e)
...
def ReloadPic():
    ...
    with con:
        cur = con.cursor(mdb.cursors.DictCursor)
        sql="select elemento from elemento where stato=1"
        cur.execute(sql)
        rows = cur.fetchall()
        for row in rows:
            elemento=row['elemento']
            sql="select mat.ip, im.pulsante, im.elemento from illuminazione.impianto im, illuminazione.matricole mat where im.elemento=%s and mat.matricola=im.matricola and mat.attiva=1;" % (elemento)
            cur.execute(sql)
            rows = cur.fetchall()
            for row in rows:
                 HOST = '%s' % row["ip"]
